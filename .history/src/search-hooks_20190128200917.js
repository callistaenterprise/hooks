  ...
  const [
    ...
    (event$, input$, state$) =>
      input$.pipe(
        ...
        debounceTime(1000),
        switchMap(([eventAndInput, state]) =>
          from(api.search(eventAndInput[1][0])).pipe(
            map(list => {
              const [
                [searchText, loading, listp, error],
                [searchTextInupt]
              ] = eventAndInput;
              console.log("----- list", {
                list,
...
export const useSearchState = () => {
  ...
  const [handleUpdateSearchText, state] = useCallApiEventObservable(s);
